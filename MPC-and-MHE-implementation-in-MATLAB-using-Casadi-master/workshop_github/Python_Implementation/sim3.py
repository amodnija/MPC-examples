...
def simulate(cat_states, cat_controls, t, step_horizon, N, reference, obsc, save=False):
    ...
    patch = plt.Circle((obsc[0][0], obsc[0][1]), obsc[0][2], fc='b')

    def init():
        patch.center = (obsc[0][0], obsc[0][1])
        ax.add_patch(patch)
        return path, horizon, current_state, target_state,


    def animate(i):
        # if len(colors)-i+1<len(colors):
        #     colors[len(colors)-i+1] = 'w'
        # if i<=len(colors) and i>0:
        #     colors[len(colors)-i] = 'k'
        ...
        return path, horizon, current_state, target_state, patch
