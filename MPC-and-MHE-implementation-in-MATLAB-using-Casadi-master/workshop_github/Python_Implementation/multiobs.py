...
for k in range(N+1):
    ...
    print('\n')
    o1 = -ca.sqrt(ca.power((X[0,k]-obs_x),2) + ca.power((X[1,k]-obs_y),2))+(obs_diam/2+rob_diam/2)
    o2 = -ca.sqrt(ca.power((X[0,k]-obs_x2),2) + ca.power((X[1,k]-obs_y2),2))+(obs_diam2/2+rob_diam/2)
    cond = -ca.sqrt(ca.power((X[0,k]-obs_x),2) + ca.power((X[1,k]-obs_y),2))-ca.sqrt(ca.power((X[0,k]-obs_x2),2) + ca.power((X[1,k]-obs_y2),2))+rob_diam/2+obs_diam2/2+obs_diam/2
    g = ca.vertcat(g, o1*o2*cond)
