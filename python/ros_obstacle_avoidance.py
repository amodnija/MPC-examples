...
N = 100              # number of look ahead steps (prediction horizon)
...
def shift_timestep(step_horizon, t0, state_init, u, f):
    ...
    print(ca.sqrt(f_value[0]**2 + f_value[1]**2),f_value[2])
    # global pox,poy
    # if pox and poy:
    #     print(pox,poy)
    ...
    return t0, next_state, u0, ca.sqrt(f_value[0]**2 + f_value[1]**2), f_value[2]
...
lbx[0: n_states*(N+1): n_states] = 0     # X lower bound [0,0+3,0+6,...till 0+3*(N+1)]
lbx[1: n_states*(N+1): n_states] = 0     # Y lower bound
...
def pose_callback(msg):
    ...
    global pox 
    pox = msg.pose.pose.position.x
    global poy 
    poy = msg.pose.pose.position.y

sub_topic_name = "/odom"
pub_topic_name = "/cmd_vel"

number_subscriber = rospy.Subscriber(sub_topic_name, Odometry, pose_callback)
pub = rospy.Publisher(pub_topic_name, Twist, queue_size=1)
velocity_msg = Twist()
...
if __name__ == '__main__':
    ...
    while (ca.norm_2(state_init - state_target) > 1e-2) and (mpc_iter * step_horizon < sim_time):
        ...
        t0, state_init, u0, lin, ang = shift_timestep(step_horizon, t0, state_init, u, f)
