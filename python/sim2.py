...
def simulate(obs_list, cat_states, cat_controls, t, step_horizon, N, reference, save=False):
    def create_triangle(state=[0,0,0], h=0.3, w=0.15, update=False):
        x, y, th = state
        triangle = np.array([
            [h, 0   ],
            [0,  w/2],
    ...
    circles = []
    for obs in obs_list:
        circles.append(plt.Circle((obs[0], obs[1]), obs[2], color='blue'))
...
#sudo codesign -f -s - --timestamp=none --all-architectures --entitlements ParallelsService.entitlements "/Applications/ParallelsDesktop.app/Contents/MacOS/Parallels Service.app/Contents/MacOS/prl_disp_service"