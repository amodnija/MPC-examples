...
R1 = 0.01              #weights for U
R2 = 0.01

step_horizon = 0.2  # time between steps in seconds (sampling time)
N = 50              # number of look ahead steps (prediction horizon)
rob_diam = 0.1        # diameter of the robot
...
for obs in obs_list:
    for k in range(N+1):
        # print(-ca.sqrt((X[1,k]-obs_x)**2 +(X[2,k]-obs_y)**2)+(obs_diam/2+rob_diam/2))
        # print('\n')
        o1 = -ca.sqrt(ca.power((X[0,k]-obs[0]),2) + ca.power((X[1,k]-obs[1]),2))+(obs[2]+rob_diam/2)
        g = ca.vertcat(g, o1)
...
lbg = ca.DM.zeros((n_states*(N+1) + len(obs_list)*(N+1), 1))
ubg = ca.DM.zeros((n_states*(N+1) + len(obs_list)*(N+1), 1))
...
lbx[0: n_states*(N+1): n_states] = -4     # X lower bound [0,0+3,0+6,...till 0+3*(N+1)]
lbx[1: n_states*(N+1): n_states] = -4     # Y lower bound
lbx[2: n_states*(N+1): n_states] = -ca.inf     # theta lower bound

ubx[0: n_states*(N+1): n_states] = 4      # X upper bound
ubx[1: n_states*(N+1): n_states] = 4      # Y upper bound
...
if __name__ == '__main__':
    ...
    # simulate
    simulate(obs_list, cat_states, cat_controls, times, step_horizon, N,
