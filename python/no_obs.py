...
step_horizon = 0.1  # time between steps in seconds (sampling time)
N = 100              # number of look ahead steps (prediction horizon)
rob_diam = 0.178      # diameter of the robot
...
if __name__ == '__main__':
    ...
    while ((ca.norm_2(state_init - state_target) > 1e-2) and (mpc_iter * step_horizon < sim_time)):
        ...
        quaternion = [orw,orx,ory,orz]
        euler = tf.transformations.euler_from_quaternion(quaternion)
        roll = euler[0]
        pitch = euler[1]
        yaw = euler[2]

        state_init = ca.DM([pox, poy, yaw])
