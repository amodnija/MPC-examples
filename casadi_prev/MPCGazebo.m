...
% tbot = turtlebot('10.20.9.87');


t0 = 0;
x0 = [0 ; 0 ; 0.0];    % initial condition.
xs = [1.5 ; 1.5 ; pi/2]; % Reference posture.
...
% value.
main_loop = tic;
while(norm((x0-xs),2) > 5e-2 && mpciter < sim_tim / T)
    ...
    % x0 = [odom.pose.pose.position.x, odom.pose.pose.position.y, 0];
    vel = u0(1,1);
    ohm = u0(1, 2);
